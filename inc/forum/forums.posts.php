...
if ($_REQUEST['post']!='') {
	...
	if ($_REQUEST['post']=='new' OR ($_REQUEST['post']=='edit' AND $_REQUEST['r']!='')) {
		...
		if ($_REQUEST['post']=='edit' AND $_SERVER['REQUEST_METHOD']!='POST') {
			$rowj=mysql_fetch_array(mysql_query("SELECT *, b.gmlevel as gmlvl
						FROM forum_posts fp
						LEFT JOIN (account a) ON a.id = fp.id_account
						LEFT JOIN (account_accesss b) ON b.id = a.id
...
document.postForm.isbbcode.checked=<?php echo $_POST['isbbcode']; ?>;
			...
			$rowj=mysql_fetch_array(mysql_query("SELECT id_post, isreply, id_account, b.gmlevel as gmlvl
			FROM forum_posts fp
			LEFT JOIN (account a) ON a.id = fp.id_account
...
$topicid =  mysql_query("SELECT *,  DATE_FORMAT(CONVERT_TZ(CONCAT(`date`, ' ', `hour`), '".$GMT[$SETTING['WEB_GMT']][0]."', '".$usergmt."'), '%d-%m-%Y') as date,
						...
						fa.displayname as dn, fa.avatar as avatar, fa.signature as signature, b.gmlevel as gmlvl, a.username
						FROM forum_posts fp
						LEFT JOIN (account a) ON a.id = fp.id_account
	...
	if ($avatar[0]!='gm' AND $avatar[0]!='mvp' AND $avatar[0]!='nochar') {
		$qquery = mysql_query("SELECT *, rs.dbhost as rsdbhost, rs.dbport as rsdbport,rs.dbuser as rsdbuser,
			...
			if (mysql_num_rows($qquery)==1) {
				...
				$newquery = @mysql_query("SELECT c.name as name, totalHonorPoints, gender, level, class, race, g.name as guild
										FROM `characters` c
				...
				if (mysql_num_rows($newquery)==1) {
					...
					$charset[0]=$rowc['name'];
					$charset[1]=$rowc['level'];
					$charset[2]=$rowc['race'];
					$charset[3]=$rowc['gender'];
					$charset[4]=$rowc['class'];
					$charset[5]=$rowg['name'];
					$charset[6]=$rowc['guild'];
					$charset[7]=$rowc['totalHonorPoints'];
				} else {
...
<div class="miniadmin">
	...
	<?php if ($rowm['gmlevel']>'0') {
			...
			$blizzreply = mysql_query("SELECT id_post, b.gmlevel as gmlvl FROM `forum_posts` fp
...
if ($rowm['id_account_edit']!='0' ) {
		...
		$ltedit=mysql_query("SELECT DATE_FORMAT(CONVERT_TZ(CONCAT(`date_edit`, ' ', `hour_edit`), '".$GMT[$SETTING['WEB_GMT']][0]."', '".$usergmt."'), '%d-%m-%Y') as date_edit, DATE_FORMAT(CONVERT_TZ(CONCAT(`date_edit`, ' ', `hour_edit`), '".$GMT[$SETTING['WEB_GMT']][0]."', '".$usergmt."'), '%h:%i %p') as  hour_edit, b.gmlevel as gmlevel, fa.displayname as dn, fa.ismvp as ismvp FROM forum_posts fp
