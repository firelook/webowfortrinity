...
<?php
		while ($rowc = mysql_fetch_array($qquery)) {
			...
			$author =  mysql_query("SELECT fp.id_post, fp.id_account, fp.`date`, fp.`hour`, b.gmlevel as gmlvl, fa.displayname as dn FROM `forum_posts` fp
			...
			$blizzreply = mysql_query("SELECT id_post FROM `forum_posts` fp
									...
									LEFT JOIN (`account_access` b) ON b.id = a.id
									WHERE fp.id_topic='".$rowc['id_topic']."' AND b.gmlevel > 0
									ORDER BY fp.id_post ASC LIMIT 0, 1", $MySQL_CON) or die (mysql_error());			
