	...
	$forceshow=true;
		...
		if ($_POST['update']=='delete') {
			...
			if ($_POST['acctype']=='normal') {
				$newquery = "SELECT a.id as id, b.gmlevel as gmlevel FROM account a LEFT JOIN (`forum_accounts` fa) ON fa.id_account = a.id LEFT JOIN (`account_access` b) ON b.id = a.id WHERE (DATEDIFF(NOW(), ".$_POST['llogo'].") >=".round($_POST['llogd'] * $_POST['llogt'], 0)." OR ".$_POST['llogo']."='0000-00-00 00:00:00')";
...
parchup(true);
	...
	$ppag=50;
	$newquery = "SELECT fa.activation as activation, a.username as username, fa.displayname as dn, fa.id_account as id, IFNULL(b.gmlevel,0) as gmlevel, 
				...
				LEFT JOIN (account_access b) ON b.id = a.id
				WHERE (b.gmlevel < '".$userlevel."' OR a.id = '".$_SESSION['userid']."' OR fa.id_account NOT IN (SELECT id FROM `account`))";
...
while($rowa = mysql_fetch_array($newquery)) {
	...
	if($res_color==1) { $res_color=2; } else { $res_color=1; }
		...
		while($rowo = @mysql_fetch_array($queryzed)) {
			...
			$newdbb = mysql_select_db ($rowo['rsdbname'], $newconb) OR DIE(mysql_error());
			$newqueryz = mysql_query("SELECT name, race, class, gender, level FROM `characters` WHERE `account`='".$rowa['id']."' ORDER BY name ASC", $newconb);
			...
			while ($rowz = @mysql_fetch_array($newqueryz)) {
				$i++;
				$tt.='<tr><td NOWRAP><span class=white>'.$rowz['name'].'</span><td></td><img src=new-hp/images/picons/'.$rowz['race'].'-'.$rowz['gender'].'.gif><td></td><img src=new-hp/images/picons/'.$rowz['class'].'.gif></td><td NOWRAP><span class=white>Lvl. '.$rowz['level'].'</span></td></tr>';
			}
...
if ($_POST['namesel']!='') { echo $pages; }
		...
		$newquery = mysql_query("SELECT *, DATE_FORMAT(`bday`,'%d/%m/%Y') as `bday`, a.username as username, a.joindate as joindate, a.last_login as last_login, 
				IFNULL(b.gmlevel,0) as gmlevel, a.email as email, a.sha_pass_hash as password FROM forum_accounts fa LEFT JOIN (account a) ON fa.id_account = a.id LEFT JOIN (account_Access b) ON b.id = a.id
				WHERE fa.id_account='".$_REQUEST['id']."' AND (b.gmlevel < '".$userlevel."' OR a.id = '".$_SESSION['userid']."' OR fa.id_account NOT IN (SELECT id FROM `account`))") OR DIE (mysql_error());
...
<table cellspacing = "0" cellpadding = "0" border = "0" width = 450>
	...
	while ($rowx = mysql_fetch_array($qquery)) {
		...
		$newquery = @mysql_query("SELECT name, gender, level, class, race, online FROM `characters` WHERE `account`='".$rowa['id_account']."'", $newcon);
		echo "<tr>
				  ...
				  <table border=0 cellspacing=0 cellpadding=0>
					...
					while($rowc = @mysql_fetch_array($newquery)) {		
						echo "<table><tr>
								...
								<td align='left'><img onmouseover='ddrivetip(\"<b>".$CHAR_RACE[$rowc['race']][0]."</b>\")' onmouseout='hideddrivetip()' src='new-hp/images/picons/".$rowc['race']."-".$rowc['gender'].".gif'></td>
								...
								<td align='left' nowrap='nowrap' width=40><small style='color: rgb(102, 13, 2);'>Lvl. ".$rowc['level']."</span></td>
