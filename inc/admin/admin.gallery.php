	...
	<div style='cursor: auto;' id='dataElement'>
		...
		for ($i=0;$i<3;$i++) {
			...
			foreach(glob("media/".str_replace(" ", "", strtolower($gal[$i]))."/*.jpg") as $filename) {
				...
				$newquery = mysql_query("SELECT username, b.gmlevel, fa.displayname as fdn FROM account a LEFT JOIN forum_accounts fa ON a.id = fa.id_account LEFT JOIN account_access b ON b.id = a.id WHERE a.id = '".$userd[0]."' GROUP BY a.id", $MySQL_CON) OR DIE (mysql_error());
