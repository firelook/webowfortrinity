...
$forceshow=true;
	...
	if ($_POST['update']=="save") {
		...
		if (strlen($_POST['wrsql_dbmangos'])<1) {
			$haserrors .="Invalid length on Trinity 'characters' Database field.<br>";		
		}
		if (strlen($_POST['wrsql_dbmangos2'])<1) {
			$haserrors .="Invalid length on Trinity 'world' Database field.<br>";		
		}
		...
		if ($haserrors=="") {
			...
			$sqldb = @mysql_select_db($_POST['wrsql_dbmangos2'], $sqlcon);			
			if (!$sqldb) $haserrors.=mysql_error().'.<br>';
			...
			if ($haserrors=="") {
				...
				if (!$newquery) $haserrors.=mysql_error().'.<br>';
				$newquery =mysql_query("INSERT INTO realm_settings(id_realm,dbhost,dbport,dbuser,dbpass,dbname,dbname2) VALUES('".$_REQUEST['id']."','".$_POST['wrsql_host']."','".$_POST['wrsql_port']."','".$_POST['wrsql_user']."','".$_POST['wrsql_pass']."','".$_POST['wrsql_dbmangos']."','".$_POST['wrsql_dbmangos2']."')", $MySQL_CON);
				if (!$newquery) $haserrors.=mysql_error().'.<br>';
			}
...
rs.dbpass as rsdbpass,rs.dbname as rsdbname, rs.dbname2 as rsdbname2 FROM `realmlist` r LEFT JOIN (realm_settings rs) ON r.id = rs.id_realm 
...
<table cellspacing = "0" cellpadding = "0" border = "0" width = "95%">
	...
	<tr>
		...
		World Datenbank:
		  ...
		  <td align=left colspan = "2"><table border=0 cellspacing=0 cellpadding=0><tr><td><input name="wrsql_dbmangos2" style = "Width:150" taborder=5/ value="<? echo $row['rsdbname2']; ?>"></td><td valign = "top">
