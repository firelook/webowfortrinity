...
<?php
	...
	while ($rowx = mysql_fetch_array($qquery)) {
		...
		$newquery = @mysql_query("SELECT guid, name, gender, level, class, race FROM `characters` WHERE `account`='".$row['id']."'", $newcon);
...
?>
	...
	<tr>
	  ...
	  <?php echo $USER_LEVEL[$row['gmlevel']]; ?><?php if ($row['id']==$SETTING['SERVER_OWNER']) { echo ' (Owner)'; } ?></span></td></tr></table></td>
	</tr>
	...
	<tr>
	  ...
	  <?php if ($row['expansion']=='2') { echo "Wrath of the Lich King"; } if ($row['expansion']=='1') { echo "The Burning Crusades"; }  if ($row['expansion']=='0') { echo "None"; }  ?>
	...
	while ($rowx = mysql_fetch_array($qquery)) {
		$newcon = @mysql_connect($rowx['rsdbhost'].':'.$rowx['rsdbport'], $rowx['rsdbuser'], $rowx['rsdbpass']);;
		$newdb = @mysql_select_db ($rowx['rsdbname'], $newcon);
		$newquery = @mysql_query("SELECT name, class, gender, level, race, online FROM `characters` WHERE `account`='".$row['id']."'", $newcon);
		echo "<tr><td colspan=5 width=100% align='left'><b><span>".$rowx['name']."</b></td></tr>";
			...
			while($rowc = @mysql_fetch_array($newquery)) {		
				echo "<tr>
						...
						<td align='left' nowrap='nowrap'><small style='color: rgb(102, 13, 2);'>Lvl. ".$rowc['level']."</span></td>
