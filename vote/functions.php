...
function chose_reward_soap($reward)
{
	...
	$remote = $remote_access[$realms[$_SESSION["realm"]][2]];
	if($tab_rewards[$reward][0])
		$command = "senditems ".$_SESSION["char_name"]." \"".$language["mail_subject"]."\" \"".$language["mail_message"]."\" ".$tab_rewards[$reward][0].":".$tab_rewards[$reward][1];
	else
		$command = "sendmoney ".$_SESSION["char_name"]." \"".$language["mail_subject"]."\" \"".$language["mail_message"]."\" ".$tab_rewards[$reward][1];
